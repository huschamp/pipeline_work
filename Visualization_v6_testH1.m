    ...
    if h==4
        ...
        for i=1:length(rangex)
            for j=1:length(rangey)
                FinY=(2.*t.^3-3.*t.^2+1).*din(1,1)+(t.^3-2.*t.^2+t).*rangey(j) +(-2.*t.^3+3.*t.^2).*din(k,1)+(t.^3-t.^2).*rangex(i);
                FinX=(2.*t.^3-3.*t.^2+1).*din(1,2)+(t.^3-2.*t.^2+t).*rangey(j) +(-2.*t.^3+3.*t.^2).*din(k,2)+(t.^3-t.^2).*rangex(i);         
                Yin=[Yin;FinY];
                Xin=[Xin;FinX];
            end;
        end;
